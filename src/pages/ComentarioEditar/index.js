...
function ComentariosEditar({ idcomentario, element }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
        ...
        delete clone._id
       const editado = await api.put(`/comentario/editar/${idcomentario}`,clone)
       console.log(editado)
    } catch (error) {
        console.log(error)
    }
    ...
    <>
      <>
        <form onSubmit={handleSubmit}>
          ...
          <textarea className="preparo mt-1 block  w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            value={comentario.content}
            onChange={handleChange}
            name="content"
          ></textarea>
          <label>Avaliação:</label>
          <select className="preparo mt-1 block   rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" name="avaliacao" onChange={handleChange}>
